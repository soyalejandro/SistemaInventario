...
function llenar_farmacia()
{
   $.ajax({
       url : 'combofarmacia.php',
       // data : {'id':id},
       type : 'POST',
       dataType : 'html',
       success : function(respuesta) {
           $("#id_farmacia").empty();
           $("#id_farmacia").html(respuesta);      
       },
       error : function(xhr, status) {
           alert('Disculpe, existió un problema');
       },
    ...
    $.ajax({
        ...
        success:function(respuesta){
            if(respuesta == "ok"){
                ...
                alertify.set('notifier','position', 'bottom-right');
                alertify.error('Ocupa Incertar farmacia');
            }
        // llenarLista();
        },
        error:function(xhr,status){
...
function imprimir(){
        ...
        var mensaje = "¿Deseas generar un archivo con PDF con la lista de los pedidos de farmacias";

    var link    = "pdfListaSurtido.php";
